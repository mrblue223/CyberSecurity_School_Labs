...
**Cloud Credential Security Architecture:**

The project used two complementary mechanisms to ensure that no long-lived AWS access keys were ever written to disk in plaintext — addressing the core threat that static credentials in `~/.aws/credentials` present a persistent compromise risk if the workstation is ever accessed by a malicious actor or automated script.

**Approach 1 — aws-vault (ByteNess Fork):**

`aws-vault` wraps the AWS CLI and injects temporary STS credentials into a subprocess, storing the underlying access keys in the OS-level keyring (GNOME Keyring on Linux, KWallet, or macOS Keychain) rather than plaintext on disk. The [ByteNess fork](https://github.com/ByteNess/aws-vault) was used for its OS keyring integration:

```bash
# Add credentials into the encrypted OS keyring — never touches ~/.aws/credentials
aws-vault add meq7-secure-profile

# Execute any AWS CLI command inside a subshell with temporary STS tokens
# Credentials are injected as environment variables, scoped to this process only
...
| Subshell isolation | Credential scope is limited to the child process |
| Automatic memory clearing | Tokens purged from memory on process termination |
| Short-lived tokens | aws-vault calls STS `AssumeRole` to generate time-limited tokens |

**Approach 2 — AWS SSO Non-Persistent Sessions:**

For operations tied to the lab's organizational account, `aws configure sso` was used. This model has no permanent access keys whatsoever — the SSO flow authenticates via browser, caches a short-lived token under `~/.aws/sso/cache/`, and re-authentication is required when the session expires:
...
The SSO wizard was completed interactively, and profile aliases were added to `.bashrc` for operational convenience without compromising the credential model:

```bash
# .bashrc alias — profile name redacted but the pattern is:
alias awslab='aws --profile [SSO_PROFILE_NAME]'
```

Session lifecycle:

```bash
# Authenticate (opens browser SSO flow)
aws sso login --profile [PROFILE]

# Verify active identity — confirms STS token, not permanent key
aws sts get-caller-identity --profile [PROFILE]

# Session expires automatically — re-run login to refresh
```

All screenshots below are from actual CLI operations performed under this SSO credential model:

![AWS SSO Configure](images/image18.png)
*`aws configure sso` — SSO session name "Lab", region `us-east-1`, SSO URL and state token redacted. The CLI automatically detected the single available account (`YulCyberClick Demo`, 453875232433) and role (`MEQ7_RBAC_Room3`) and configured the profile accordingly.*

![AWS SSO Access Portal](images/image17.png)
*AWS Access Portal — `YulCyberClick Demo` account under `MEQ7_RBAC_Room3`. The **"Access keys"** link generates temporary STS credentials on demand. No permanent IAM user access keys exist at any point in this workflow.*

![AWS SSO Login Success](images/image11.png)
*`aws sso login --profile [REDACTED]` — successful browser-based SSO authentication. The CLI prints the SSO authorization URL and waits for browser confirmation. Profile name and URL redacted for operational security.*

![AWS STS Caller Identity](images/image15.png)
*`aws sts get-caller-identity --profile [REDACTED]` — confirms the active session is a temporary STS token (not a permanent IAM key). UserId, Account number, and ARN are redacted. This command is the canonical way to verify no long-lived credentials are in use — a permanent key would return a different ARN format (`arn:aws:iam::ACCOUNT:user/USERNAME` vs the STS assumed-role format seen here).*

**Why this matters for the project:**

Both aws-vault and SSO-based sessions enforce a zero-standing-credential posture. If the Kali workstation were compromised, an attacker would find no extractable AWS keys in `~/.aws/credentials` — only encrypted keyring entries (for aws-vault) or expired SSO cache tokens (for SSO sessions). This directly mitigates the most common AWS credential compromise vector: credential file theft.
